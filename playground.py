...
def set_up_browser(machine_name):
    ...
    driver = webdriver.Remote(
        command_executor="http://192.168.196.159:4444/wd/hub",
...
def get_post_data(link):
    # The ID and range of a sample spreadsheet.
    SAMPLE_SPREADSHEET_ID = '1eBxVRTIfnHO1miRg6grcNrprMGDZ7dvIGrEoFNDTxio'
    SAMPLE_RANGE_NAME = 'PostData!2:1000'
...
def post(listing_data, driver):
    ...
    WebDriverWait(driver, 10).until(
        EC.element_to_be_clickable((By.CSS_SELECTOR, '#post'))).click()
    ...
    if category == 'vacation rentals':
        ...
        driver.find_element(By.CSS_SELECTOR, '#ui-id-2-menu').send_keys(fixed_keys(post_data[11]))
        try:
            driver.find_element(By.CSS_SELECTOR, '#ui-id-2-menu').send_keys(Keys.ENTER)
        except:
            pass
        ...
        driver.find_element(By.CSS_SELECTOR, '#ui-id-3-menu').send_keys(fixed_keys(post_data[12]))
        try:
        ...
        driver.find_element(By.CSS_SELECTOR, '#ui-id-4-menu').send_keys(post_data[13])
        try:
        ...
        driver.find_element(By.CSS_SELECTOR, '#ui-id-5-menu').send_keys(post_data[14])
        try:
    ...
    if category == 'rooms & shares' or category == 'sublets & temporary':
        ...
        # Laundry
        driver.find_element(By.CSS_SELECTOR, '#ui-id-4-button .ui-selectmenu-text').click()
        driver.find_element(By.CSS_SELECTOR, '#ui-id-4-menu').send_keys(fixed_keys(post_data[11]))
        try:
            driver.find_element(By.CSS_SELECTOR, '#ui-id-4-menu').send_keys(Keys.ENTER)
        except:
            pass
        ...
        if post_data[26]:
            driver.find_element(By.CSS_SELECTOR, '#ui-id-2-button .ui-selectmenu-text').click()
            driver.find_element(By.CSS_SELECTOR, '#ui-id-2-menu').send_keys(fixed_keys(post_data[26]))
            try:
                driver.find_element(By.CSS_SELECTOR, '#ui-id-2-menu').send_keys(Keys.ENTER)
            except:
                pass
        ...
        # Parking
        driver.find_element(By.CSS_SELECTOR, '#ui-id-5-button .ui-selectmenu-text').click()
        driver.find_element(By.CSS_SELECTOR, '#ui-id-5-menu').send_keys(fixed_keys(post_data[12]))
        try:
            driver.find_element(By.CSS_SELECTOR, '#ui-id-5-menu').send_keys(Keys.ENTER)
        except:
            pass
        ...
        # Bedrooms
        driver.find_element(By.CSS_SELECTOR, '#ui-id-6-button .ui-selectmenu-text').click()
        driver.find_element(By.CSS_SELECTOR, '#ui-id-6-menu').send_keys(post_data[13])
        try:
        ...
        driver.find_element(By.CSS_SELECTOR, '#ui-id-7-menu').send_keys(post_data[14])
        try:
        ...
        # Rent Period
        if post_data[15]:
            driver.find_element(By.CSS_SELECTOR, '#ui-id-9-button .ui-selectmenu-text').click()
            driver.find_elements(By.XPATH, f"//*[contains(text(), '{post_data[15]}')]")[1].click()
...
def main():
    ...
    for row_num, task in enumerate(tasks):
        if len(task) != 6:
            ...
            try:
                driver = set_up_browser(computers[task[-1]])
                if task[0] == 'Repost':
